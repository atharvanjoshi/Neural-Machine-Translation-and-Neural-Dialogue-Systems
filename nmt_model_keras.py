...
class AttentionLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    Start
    """
    # This attention mechanism calculates the score between decoder_outpus and encoder_outputs using matrix multiplication
    # However, we need to transpose the last two dimensions of decoder_outputs and that is done as follows:
    decoder_outputs_t = K.permute_dimensions(decoder_outputs,(0,2,1))
    #Luong score is calculated by the dot product of encoder outputs and the transposed decoder output.
    luong_score = Softmax(axis=1)(K.batch_dot(encoder_outputs,decoder_outputs_t))
    #Finally we expand the dimensions so that both have the same shape.
    encoder_vector = K.sum((K.expand_dims(encoder_outputs,axis=2)*K.expand_dims(luong_score, axis=-1)),axis=1)
...
class NmtModel(object):
  ...
  def build(self):
    ...
    #Here we creating two embedding layers - one for source and one for the target
    #Both the layers have their mask_zero set to True to remove the padding.
    #The input dimension is the vocabulary size of source and target respectively.
    #The output dimension for both is the embedding size defined previously.
    #The embedding initializer is 'random_uniform' since we want randomly initialised embedding lookup
    #Finally we the input layers of both source and target to their respective embedding layers.
    source_embeddings = Embedding(mask_zero = True, input_dim = self.vocab_source_size, output_dim = self.embedding_size, embeddings_initializer='random_uniform')(source_words)
    target_embeddings = Embedding(mask_zero = True, input_dim = self.vocab_target_size, output_dim = self.embedding_size, embeddings_initializer='random_uniform')(target_words)
    ...
    #Here we are creating two dropout layers one for source and one for target
    #The dropout rate is same as defined before.
    #Finally we pass the embedding layers created previously to the respective dropout layers.
    source_word_embeddings = Dropout(self.embedding_dropout_rate, input_shape=(None,))(source_embeddings)

    target_words_embeddings = Dropout(self.embedding_dropout_rate, input_shape=(None,))(target_embeddings)

    # (c.) An encoder LSTM() with return sequences set to True
    print('\nTask 1(c): Creating an encoder')
    #Here we are creating an encoder that uses LSTM where the hidden size and hidden dropout rate are as given. The return sequences and return state are true.
    ...
    #Appending the decoder state input h and input c to a list.
    decoder_states = [decoder_state_input_h,decoder_state_input_c]
    # use decoder states as input to the decoder lstm to get the decoder outputs, h, and c for test time inference
    #Creating a decoder LSTM which tales the target word embeddings and the list of decoder states that we defined before this as the intial state.
    ...
    #Since we are trying both with and without attention, if we want to use the attention layer, we can define it as below
    #The layer takes the list of encoder output input and decoder output test as parameter.
    if self.use_attention:
      decoder_attention = AttentionLayer()
    ...
    #Finally here we pass the decoder output test (with or without attention) to the decoder dense layer.
  ...
  def eval(self, dataset):
    ...
    #This shows the predicted sentence and the actual sentence.
    #The output can be visualised on the Jupyter Notebook.
    print("*******SAMPLE OUTPUT**********")
    for i in range(5):
      print("{} Sample".format(i+1))
      print("Sentence predicted by model: "+" ".join(candidates[i]))
